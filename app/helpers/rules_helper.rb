module RulesHelper
  ...
  # Link helpers
  #
  def setter_name(command, i)
    "#{h(command)} <sup>(#{h(@command_counts[i])})</sup>".html_safe
  end

  def setter_href(command)
  ...
  def current_setters(rule)
    if params[:commands].present?
      setters = params[:commands].zip(params[:args]).map do |command, arg|
        Rule.new(command: command, arg: arg)
      end
    end
    setters ||= rule.new_record? ? [Rule.new] : rule.setters
  end
end
